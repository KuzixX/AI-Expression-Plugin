...
def assign_target_to_source_clusters(verts_target, heat_target_per_anchor,
                                       src_clusters_list, heat_threshold=0.05,
                                       geodesic_factor=3.0):
    """Voronoi-разбиение зон головы 2 по центроидам source-кластеров +
    геодезический лимит через spatial_sigma.

    Геодезик проверяется в ДВА уровня:
      1) Belonging to zone — heat_target[anchor, v] > heat_threshold · max
         Это honest геодезик: heat диффузия следует поверхности.
      2) Within cluster reach — ||v - c_rest_source|| < σ · geodesic_factor
         Где σ = spatial_sigma source-кластера (RMS его пространственного
         разброса на голове 1). Euclidean — но ограничено радиусом кластера.

    Вершины не попавшие ни в одну "досягаемость" — НЕ приписываются никуда
    (не получают деформацию).

    geodesic_factor:
      3.0 (default) — мягкий лимит, кластер охватывает ~99.7% области (3σ)
      2.0 — строже, только 95%
      1.5 — очень строго, только 87%
      бесконечность — отключить лимит, всё приписывается к ближайшему центроиду
    """
    target_clusters = []
    ...
    for a, src_list in src_by_anchor.items():
        ...
        active_verts = verts_target[active_idx]                          # (M, 3)
        centroids    = np.stack([s['c_rest']        for s in src_list])  # (K, 3)
        sigmas       = np.array( [s['spatial_sigma'] for s in src_list]) # (K,)
        dists        = np.linalg.norm(
            active_verts[:, None, :] - centroids[None, :, :], axis=-1
        )                                                                 # (M, K)
        # Ограничение по геодезическому радиусу: ставим dist = inf если v вне
        # достижимости кластера.
        max_dist = sigmas * geodesic_factor                              # (K,)
        unreachable = dists > max_dist[None, :]
        dists_capped = np.where(unreachable, np.inf, dists)              # (M, K)
        # Ближайший достижимый кластер
        nearest = np.argmin(dists_capped, axis=1)                        # (M,)
        # Вершины у которых ВСЕ кластеры недостижимы — пропускаем
        reachable = ~np.all(unreachable, axis=1)
